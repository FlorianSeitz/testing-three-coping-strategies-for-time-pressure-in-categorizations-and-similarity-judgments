...
class AdjustableStimulus:
    def __init__(self, instruction = "no", offset_y = 75, offset_x=100, pos=1, forced_choice=False):
        self.docu = []
        self.my_control = 0
        self.canvas_stim, self.response_rect = self.get_canvas(pos, offset_x, offset_y, forced_choice, instruction)
        self.u = grid_width
    ...
    def get_canvas(self, pos, offset_x, offset_y, forced_choice, instruction="no"):
        if instruction == "no":
            canvas_stim = stimuli.Canvas((control.defaults.window_size[0]/2, control.defaults.window_size[1]), (0, 200), colour=misc.constants.C_WHITE)
        else:
            canvas_stim = stimuli.Canvas((control.defaults.window_size[0]/2, 350), (0, -50), colour=misc.constants.C_WHITE)
        response_rect = 0
    ...
    def create_shape_cross(self, pos, rotate, color, height=grid_height, width=grid_width):
        shape = stimuli.FixCross((width, height), pos, (grid_width+grid_height)/4, color)
        shape.rotate(rotate)
        shape.reposition(pos)
        return shape
    ...
    def create_cell(self, pos, rotate):
        r = stimuli.Rectangle((grid_width, grid_height), misc.constants.C_WHITE,  1, position=(0,0))
        r.rotate(rotate)
        r.reposition(new_position=pos)
        self.grid.append(r)
        r_in = stimuli.Rectangle((grid_width, grid_height), misc.constants.C_GREY,  0, position=(0,0))
        r_in.rotate(rotate)
        r_in.reposition(new_position=pos)
        return r_in
    
    
    def create_cues(self, deg, cells, offset =(0,0)):
        i = 0
        alpha = (3.14/180)*(deg)
...
#        pos1 = (150, 30, 270)
        ...
        ##### Florian change here
#        pos2 = ((18.3, 24),(-16.5, 24), (0,-(1.732*grid_height/2)-grid_height/2))
        pos2 = ((23, 15),(-7, 38), (0,-(1.732*grid_height/2)-grid_height/2))
        # Position of shapes within grid. You should not need to change it
#        pos3 = (150-90,270+30,0)
        pos3 = (150-75,270+45,15)

        pal = [pygame.Color("#FF29C3"),pygame.Color("#FFDD29"), pygame.Color("#1ECBCC")] # lightred: #FF70D7
        col_1 = pal[pal_set[0]]
        col_2 = pal[pal_set[1]]
        col_3 = pal[pal_set[2]]
        self.create_cues(pos1[pos_set[0]], self.cells1, pos2[pos_set[0]])
        self.create_cues(pos1[pos_set[1]], self.cells2, pos2[pos_set[1]])
        self.create_cues(pos1[pos_set[2]], self.cells3, pos2[pos_set[2]])
        ...
        [pos_docu[pos_set[1]], pal_docu[pal_set[1]], self.size2+1, "rectangle"],
        [pos_docu[pos_set[2]], pal_docu[pal_set[2]], self.size3+1, "rectangle"]]
