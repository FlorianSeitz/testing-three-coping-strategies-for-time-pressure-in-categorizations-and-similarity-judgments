ranking <- function(weights, models, crit, weighted = FALSE, exclude = FALSE) {
  ...
  comparisons <- rbindlist(apply(pairs, 1, function(x) { # compares models pairwise
    ...
    models_x[, cond := weights$cond]
  }))
  comparisons <- comparisons[, .(sum = round(sum(m1_better_m2), 2)), by = list(cond, m1)]
  setkey(comparisons, cond, sum) # rank order (higher number = better)
  comparisons
}
