ranking <- function(weights, models, crit, weighted = FALSE, exclude = FALSE) {
  ...
  comparisons <- rbindlist(apply(pairs, 1, function(x) { # compares models pairwise
    ...
    models_x[, time_pressure := weights$time_pressure]
  }))
  comparisons <- comparisons[, .(sum = round(sum(m1_better_m2), 2)), by = list(time_pressure, m1)]
  setkey(comparisons, time_pressure, sum) # rank order (higher number = better)
  comparisons
}
